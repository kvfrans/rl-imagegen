...
class ActorNetwork(object):
    ...
    def __init__(self, sess, state_dim, action_dim, action_bound, learning_rate, tau):
        ...
        # Actor Network
        self.inputs, self.out, self.scaled_out = self.create_actor_network("actor_real")
        self.network_params = [var for var in tf.trainable_variables() if 'actor_real' in var.name]

        # Target Network
        self.target_inputs, self.target_out, self.target_scaled_out = self.create_actor_network("actor_target")
        self.target_network_params = [var for var in tf.trainable_variables() if 'actor_target' in var.name]
...
class CriticNetwork(object):
    ...
    def __init__(self, sess, state_dim, action_dim, learning_rate, tau):
        ...
        self.inputs, self.action, self.out = self.create_critic_network("critic_real")
        self.network_params = [var for var in tf.trainable_variables() if 'critic_real' in var.name]

        # Target Network
        self.target_inputs, self.target_action, self.target_out = self.create_critic_network("critic_target")
        self.target_network_params = [var for var in tf.trainable_variables() if 'critic_target' in var.name]
...
def train(sess, env, actor, critic):
    ...
    for i in xrange(MAX_EPISODES):
        ...
        for j in xrange(199):
            ...
            reshaped_state = np.expand_dims(s, 0)
            a = actor.predict(reshaped_state)[0]
            a += (1.0 / (1.0 + i + j))
            ...
            if terminal:
                ...
                print '| Reward: %d' % int(ep_reward), " | Episode", i, \
